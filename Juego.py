...
import pygame, sys # Importamos la librería Pygame, que se utilizará para crear el juego y el módulo sys que proporciona herramientas útiles para trabajar con el intérprete de Python y el sistema en el que se ejecuta
...
def start_skill():
    mensaje_bienvenida = 'Bienvenido al videojuego versión 1'
...
@ask.intent('GameIntent') # Se indica que la función que sigue es la que manejará las solicitudes del intent HelloWorldIntent 
def hello_world(): # Definimos la función que será ejecutada
    ...
    text = font.render ('Versión 1', True, (255, 255, 255)) # Creamos una superficie de texto que contiene la cadena "Hola Mundo" en blanco (255, 255, 255) sobre un fondo negro
    ...
    return statement('Versión 1') # Devolvemos una respuesta a la solicitud de Alexa con la cadena "Hola Mundo"
